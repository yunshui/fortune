...
# 导入A股数据获取模块
from data_services.a_share_finance import get_sse_index_data
...
class SSE_Predictor:
    ...
    def fetch_data(self):
        ...
        # 获取上证指数数据（2年数据以确保MA250等长期指标有足够数据）
        print("  - 上证指数数据...")
        self.sse_data = get_sse_index_data(period_days=730)

        if self.sse_data is None or self.sse_data.empty:
            raise ValueError("上证指数数据获取失败")

        print(f"  ✅ 数据获取完成（上证：{len(self.sse_data)} 条）")
    ...
    def calculate_features(self):
        ...
        sse_df = self.calculate_technical_indicators(self.sse_data)
        ...
        for period in periods:
            if len(sse_df) >= period:
                return_col = f'Return_{period}d'
                sse_df[return_col] = sse_df['Close'].pct_change(period)

                # 计算趋势方向（1=上涨，0=下跌）
                trend_col = f'{period}d_Trend'
                sse_df[trend_col] = (sse_df[return_col] > 0).astype(int)

                # 计算相对强度信号（基于收益率和MA250）
                rs_signal_col = f'{period}d_RS_Signal'
                sse_df[rs_signal_col] = (sse_df[return_col] > 0).astype(int)

        # 计算多周期MA250趋势
        for period in periods:
            if len(sse_df) >= period:
                trend_col = f'{period}d_Trend_MA250'
                sse_df[trend_col] = (sse_df['MA250'].diff(period) > 0).astype(int)

                # 计算MA250的相对强度信号
                rs_signal_col = f'{period}d_RS_Signal_MA250'
                sse_df[rs_signal_col] = (sse_df['Close'] > sse_df['MA250']).astype(int)

        # 计算多周期Volume_MA250趋势
        for period in periods:
            if len(sse_df) >= period:
                trend_col = f'{period}d_Trend_Volume_MA250'
                sse_df[trend_col] = (sse_df['Volume_MA250'].diff(period) > 0).astype(int)

                # 计算Volume_MA250的相对强度信号
                rs_signal_col = f'{period}d_RS_Signal_Volume_MA250'
                sse_df[rs_signal_col] = (sse_df['Volume'] > sse_df['Volume_MA250']).astype(int)

        # 计算多周期MA120趋势
        for period in periods:
            if len(sse_df) >= period:
                trend_col = f'{period}d_Trend_MA120'
                sse_df[trend_col] = (sse_df['MA120'].diff(period) > 0).astype(int)

        # 计算120日波动率
        sse_df['Volatility_120d'] = sse_df['Return_1d'].rolling(window=120).std()

        # 获取最新数据（最近一天）
        latest_sse = sse_df.iloc[-1]
        ...
        self.features = {
            # 长期移动平均线相关
            'MA250': safe_get(latest_sse.get('MA250', latest_sse['Close'])),
            'Volume_MA250': safe_get(latest_sse.get('Volume_MA250', latest_sse['Volume'])),
            'MA120': safe_get(latest_sse.get('MA120', latest_sse['Close'])),

            # 多周期相对强度信号（RS_Signal）
            '60d_RS_Signal_MA250': safe_get(latest_sse.get('60d_RS_Signal_MA250', 0), 0),
            '60d_RS_Signal_Volume_MA250': safe_get(latest_sse.get('60d_RS_Signal_Volume_MA250', 0), 0),
            '20d_RS_Signal_MA250': safe_get(latest_sse.get('20d_RS_Signal_MA250', 0), 0),
            '10d_RS_Signal_MA250': safe_get(latest_sse.get('10d_RS_Signal_MA250', 0), 0),
            '5d_RS_Signal_MA250': safe_get(latest_sse.get('5d_RS_Signal_MA250', 0), 0),
            '3d_RS_Signal_MA250': safe_get(latest_sse.get('3d_RS_Signal_MA250', 0), 0),

            # 多周期趋势（Trend）
            '60d_Trend_MA250': safe_get(latest_sse.get('60d_Trend_MA250', 0), 0),
            '20d_Trend_MA250': safe_get(latest_sse.get('20d_Trend_MA250', 0), 0),
            '10d_Trend_MA250': safe_get(latest_sse.get('10d_Trend_MA250', 0), 0),
            '5d_Trend_MA250': safe_get(latest_sse.get('5d_Trend_MA250', 0), 0),
            '3d_Trend_MA250': safe_get(latest_sse.get('3d_Trend_MA250', 0), 0),

            # 成交量趋势
            '60d_Trend_Volume_MA250': safe_get(latest_sse.get('60d_Trend_Volume_MA250', 0), 0),
            '20d_Trend_Volume_MA250': safe_get(latest_sse.get('20d_Trend_Volume_MA250', 0), 0),

            # 波动率
            'Volatility_120d': safe_get(latest_sse.get('Volatility_120d', 0), 0),

            # 中期均线趋势
            '60d_Trend_MA120': safe_get(latest_sse.get('60d_Trend_MA120', 0), 0),

            # 成交量相对强度
            '20d_RS_Signal_Volume_MA250': safe_get(latest_sse.get('20d_RS_Signal_Volume_MA250', 0), 0),
        }
    ...
    def normalize_feature(self, feature_name, value):
        ...
        # Level特征
            ...
            # 指数水平通常在2000-4000之间，标准化到[0, 1]
            if pd.isna(value):
                return 0
            return (value - 3000) / 1000  # 3000为中位数
    ...
    def generate_report(self):
        ...
        try:
            ...
            # 获取当前价格和日期
            current_price = self.sse_data['Close'].iloc[-1]
            previous_price = self.sse_data['Close'].iloc[-2]
            price_change = ((current_price - previous_price) / previous_price) * 100
            current_date = self.sse_data.index[-1].strftime('%Y-%m-%d')
            ...
            report_file = os.path.join(output_dir, f'sse_prediction_report_{timestamp}.json')
            ...
            features_csv = os.path.join(data_dir, f'sse_prediction_features_{timestamp}.csv')
    ...
    def print_report(self, current_price, price_change, prediction_score, prediction_trend, feature_details):
        ...
        print(f"📊 上指收盘：{current_price:.2f}（{price_change:+.2f}%）")
        ...
        # 移动平均线
        ma5 = latest_sse.get('MA5', latest_sse['Close'])
        ma10 = latest_sse.get('MA10', latest_sse['Close'])
        ma20 = latest_sse.get('MA20', latest_sse['Close'])
        ma60 = latest_sse.get('MA60', latest_sse['Close'])
        ma120 = latest_sse.get('MA120', latest_sse['Close'])
        ma250 = latest_sse.get('MA250', latest_sse['Close'])
        ...
        # MACD指标
        macd = latest_sse.get('MACD')
        macd_signal = latest_sse.get('MACD_Signal')
        macd_hist = latest_sse.get('MACD_Histogram')
        ...
        # 布林带
        bb_upper = latest_sse.get('BB_Upper')
        bb_middle = latest_sse.get('BB_Middle')
        bb_lower = latest_sse.get('BB_Lower')
        bb_position = latest_sse.get('BB_Position')
        ...
        # 成交量
        volume = latest_sse.get('Volume', 0)
        volume_ma20 = latest_sse.get('Volume_MA250', 0)
        volume_ratio = latest_sse.get('Volume_Ratio')
        ...
        # ATR (真实波幅)
        atr = latest_sse.get('ATR')
        if pd.notna(atr):
            atr_pct = (atr / current_price) * 100
            print(f"\n📊 ATR (平均真实波幅): {atr:.2f} ({atr_pct:.2f}%)")

        # 波动率
        volatility_120d = latest_sse.get('Volatility_120d', 0) * 100
        print(f"📊 120日波动率: {volatility_120d:.2f}%")

        # 支撑阻力位
        resistance = latest_sse.get('Resistance_120d', 0)
        support = latest_sse.get('Support_120d', 0)
